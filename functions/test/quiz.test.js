...
test("levelForHilals keeps early levels attainable", () => {
  ...
  assert.equal(t.MAX_LEVEL, 20);
  assert.equal(t.GOLDEN_CRESCENT_WEEKS, 10);
  assert.equal(t.levelForHilals(900).level, 10);
  assert.equal(t.levelForHilals(900).maxLevel, false);
  assert.equal(t.levelForHilals(3325).level, 20);
  assert.equal(t.levelForHilals(3325).maxLevel, true);
  ...
  assert.equal(t.levelForHilals(t.hilalsFloorForLevel(20)).level, 20);
  assert.equal(t.titleForLevel(1), "Çömez");
  assert.equal(t.cosmeticsForLevel(5).title, "Hatip");
  assert.equal(t.cosmeticsForLevel(9).title, "Müderris");
  assert.equal(t.cosmeticsForLevel(10).title, "Derviş");
  assert.equal(t.cosmeticsForLevel(20).title, "Şeyhülislam");
  ...
  assert.equal(t.cosmeticsForLevel(11).avatarFrameTier, 3);
  ...
  assert.equal(t.cosmeticsForLevel(13).hilalPulse, true);
  assert.equal(t.cosmeticsForLevel(17).nameAccentGilt, true);
  assert.equal(t.championWeeksOf({ hilalChampionWeeks: 10 }), 10);
